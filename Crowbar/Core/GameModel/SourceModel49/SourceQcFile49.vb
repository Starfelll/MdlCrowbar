...
#Region "Methods"
	...
	Public Sub WriteTextureFileNameComments()
		...
		If Settings.DecompilerSettings.DecompileDebugInfoFilesIsChecked AndAlso Me.theMdlFileData.theTextures IsNot Nothing Then
			Dim line As String
		...
		If Me.theMdlFileData.theAttachments IsNot Nothing Then
			...
			For attachmentIndex As Integer = 0 To Me.theMdlFileData.theAttachments.Count - 1
				...
				If anAttachment.theName = "" Then
					line += attachmentIndex.ToString(Settings.InternalNumberFormat)
				Else
					line += """"
	...
	Public Sub WriteMaxEyeDeflectionCommand()
		...
		If Me.theMdlFileData.maxEyeDeflection < -0.0000001 OrElse Me.theMdlFileData.maxEyeDeflection > 0.0000001 Then
			...
			Else
				line = "$maxeyedeflection "
			End If
			line += deflection.ToString("0.######", Settings.InternalNumberFormat)
			Me.theOutputFileStreamWriter.WriteLine(line)
		End If
	End Sub
	...
	Private Sub WriteSectionFramesCommand()
		...
		If Me.theMdlFileData.theSectionFrameCount > 0 Then
			...
			End If
			line += " "
			line += Me.theMdlFileData.theSectionFrameCount.ToString(Settings.InternalNumberFormat)
			line += " "
			line += Me.theMdlFileData.theSectionFrameMinFrameCount.ToString(Settings.InternalNumberFormat)
			Me.theOutputFileStreamWriter.WriteLine(line)
		End If
	End Sub
	...
	Private Sub WriteSequenceOptions(ByVal aSequenceDesc As SourceMdlSequenceDesc)
		...
		If aSequenceDesc.theAnimTags IsNot Nothing Then
			For Each animTag As SourceMdlAnimTag In aSequenceDesc.theAnimTags
				...
				line += """ "
				line += animTag.cycle.ToString(Settings.InternalNumberFormat)
				Me.theOutputFileStreamWriter.WriteLine(line)
			Next
		End If
		...
		If aSequenceDesc.groupSize(0) <> aSequenceDesc.groupSize(1) Then
			line = vbTab
			line += "blendwidth "
			line += aSequenceDesc.groupSize(0).ToString(Settings.InternalNumberFormat)
			Me.theOutputFileStreamWriter.WriteLine(line)
		End If
	...
	Private Sub WriteCmdListOptions(ByVal aSequenceDesc As SourceMdlSequenceDesc, ByVal anAnimationDesc As SourceMdlAnimationDesc49, ByVal impliedAnimDesc As SourceMdlAnimationDesc49)
		...
		If anAnimationDesc.theIkRules IsNot Nothing Then
			...
			For Each anIkRule As SourceMdlIkRule In anAnimationDesc.theIkRules
				...
				line += " contact "
				line += tempInteger.ToString(Settings.InternalNumberFormat)

				line += " fakeorigin "
				line += anIkRule.pos.x.ToString("0.##", Settings.InternalNumberFormat)
				line += " "
				line += anIkRule.pos.y.ToString("0.##", Settings.InternalNumberFormat)
				line += " "
				line += anIkRule.pos.z.ToString("0.##", Settings.InternalNumberFormat)

				Dim angles As SourceVector
				angles = MathModule.ToEulerAngles(anIkRule.q)
				line += " fakerotate "
				line += angles.x.ToString("0.##", Settings.InternalNumberFormat)
				line += " "
				line += angles.y.ToString("0.##", Settings.InternalNumberFormat)
				line += " "
				line += angles.z.ToString("0.##", Settings.InternalNumberFormat)

				line += " floor "
				line += anIkRule.floor.ToString("0.##", Settings.InternalNumberFormat)

				line += " height "
				line += anIkRule.height.ToString("0.##", Settings.InternalNumberFormat)
				...
				'pRule->radius = verify_atof( token );
				line += " radius "
				line += anIkRule.radius.ToString("0.##", Settings.InternalNumberFormat)
				...
				line += " target "
				line += anIkRule.slot.ToString(Settings.InternalNumberFormat)
			...
			For j As Integer = 0 To aSeqDesc.theAutoLayers.Count - 1
				...
				Else
					...
					If (layer.flags And SourceMdlAutoLayer.STUDIO_AL_POSE) = 0 Then
						...
						'End While
						influenceStartText = influenceStart.ToString("0", Settings.InternalNumberFormat)
						...
						'End While
						influencePeakText = influencePeak.ToString("0", Settings.InternalNumberFormat)
						...
						'End While
						influenceTailText = influenceTail.ToString("0", Settings.InternalNumberFormat)
						...
						'	influenceEnd -= endFrameIndex
						'End While
						influenceEndText = influenceEnd.ToString("0", Settings.InternalNumberFormat)
					Else
						influenceStartText = layer.influenceStart.ToString("0.######", Settings.InternalNumberFormat)
						influencePeakText = layer.influencePeak.ToString("0.######", Settings.InternalNumberFormat)
						influenceTailText = layer.influenceTail.ToString("0.######", Settings.InternalNumberFormat)
						influenceEndText = layer.influenceEnd.ToString("0.######", Settings.InternalNumberFormat)
					End If
					line += " "
	...
	Private Sub WriteCmdListLocalHierarchyOption(ByVal anAnimationDesc As SourceMdlAnimationDesc49)
		...
		If anAnimationDesc.theLocalHierarchies IsNot Nothing Then
			For hierarchyIndex As Integer = 0 To anAnimationDesc.theLocalHierarchies.Count - 1
				...
				line += """"
				line += " range "
				line += startInfluence.ToString("0.######", Settings.InternalNumberFormat)
				line += " "
				line += peakInfluence.ToString("0.######", Settings.InternalNumberFormat)
				line += " "
				line += tailInfluence.ToString("0.######", Settings.InternalNumberFormat)
				line += " "
				line += endInfluence.ToString("0.######", Settings.InternalNumberFormat)
				Me.theOutputFileStreamWriter.WriteLine(line)
			Next
		End If
	End Sub
	...
	Private Sub WriteCollisionModelOrCollisionJointsOptions()
		...
		For i As Integer = 0 To Me.thePhyFileData.theSourcePhyPhysCollisionModels.Count - 1
			...
			If Me.thePhyFileData.theSourcePhyRagdollConstraintDescs.ContainsKey(aSourcePhysCollisionModel.theIndex) Then
				...
				line += " "
				line += xFrictionValue.ToString("0.######", Settings.InternalNumberFormat)
				Me.theOutputFileStreamWriter.WriteLine(line)
				line = vbTab
				line += "$jointconstrain """
				line += aSourcePhysCollisionModel.theName
				...
				line += " "
				line += yFrictionValue.ToString("0.######", Settings.InternalNumberFormat)
				Me.theOutputFileStreamWriter.WriteLine(line)
				line = vbTab
				line += "$jointconstrain """
				line += aSourcePhysCollisionModel.theName
				...
				line += " "
				line += zFrictionValue.ToString("0.######", Settings.InternalNumberFormat)
				Me.theOutputFileStreamWriter.WriteLine(line)
			End If
		Next
		...
		For i As Integer = 0 To Me.theMdlFileData.theBones.Count - 1
			...
			If aBone.proceduralRuleType = SourceMdlBone.STUDIO_PROC_JIGGLE AndAlso aBone.proceduralRuleOffset <> 0 Then
				...
				If (aBone.theJiggleBone.flags And SourceMdlJiggleBone.JIGGLE_IS_FLEXIBLE) > 0 Then
					...
					line += vbTab
					line += "length "
					line += aBone.theJiggleBone.length.ToString("0.######", Settings.InternalNumberFormat)
					Me.theOutputFileStreamWriter.WriteLine(line)
					line = vbTab
					line += vbTab
					line += "tip_mass "
					line += aBone.theJiggleBone.tipMass.ToString("0.######", Settings.InternalNumberFormat)
					Me.theOutputFileStreamWriter.WriteLine(line)

					line = vbTab
					line += vbTab
					line += "pitch_stiffness "
					line += aBone.theJiggleBone.pitchStiffness.ToString("0.######", Settings.InternalNumberFormat)
					Me.theOutputFileStreamWriter.WriteLine(line)
					line = vbTab
					line += vbTab
					line += "pitch_damping "
					line += aBone.theJiggleBone.pitchDamping.ToString("0.######", Settings.InternalNumberFormat)
					Me.theOutputFileStreamWriter.WriteLine(line)
					line = vbTab
					line += vbTab
					line += "yaw_stiffness "
					line += aBone.theJiggleBone.yawStiffness.ToString("0.######", Settings.InternalNumberFormat)
					Me.theOutputFileStreamWriter.WriteLine(line)
					line = vbTab
					line += vbTab
					line += "yaw_damping "
					line += aBone.theJiggleBone.yawDamping.ToString("0.######", Settings.InternalNumberFormat)
					Me.theOutputFileStreamWriter.WriteLine(line)
					...
					line += aBone.theJiggleBone.alongStiffness.ToString("0.######", Settings.InternalNumberFormat)
					Me.theOutputFileStreamWriter.WriteLine(line)
					line = vbTab
					line += vbTab
					line += "along_damping "
					line += aBone.theJiggleBone.alongDamping.ToString("0.######", Settings.InternalNumberFormat)
					Me.theOutputFileStreamWriter.WriteLine(line)
				...
				If (aBone.theJiggleBone.flags And SourceMdlJiggleBone.JIGGLE_HAS_BASE_SPRING) > 0 Then
					...
					line += vbTab
					line += "base_mass "
					line += aBone.theJiggleBone.baseMass.ToString("0.######", Settings.InternalNumberFormat)
					Me.theOutputFileStreamWriter.WriteLine(line)
					line = vbTab
					line += vbTab
					line += "stiffness "
					line += aBone.theJiggleBone.baseStiffness.ToString("0.######", Settings.InternalNumberFormat)
					Me.theOutputFileStreamWriter.WriteLine(line)
					line = vbTab
					line += vbTab
					line += "damping "
					line += aBone.theJiggleBone.baseDamping.ToString("0.######", Settings.InternalNumberFormat)
					Me.theOutputFileStreamWriter.WriteLine(line)
					...
					line += aBone.theJiggleBone.baseMinLeft.ToString("0.######", Settings.InternalNumberFormat)
					line += " "
					line += aBone.theJiggleBone.baseMaxLeft.ToString("0.######", Settings.InternalNumberFormat)
					Me.theOutputFileStreamWriter.WriteLine(line)
					line = vbTab
					line += vbTab
					line += "left_friction "
					line += aBone.theJiggleBone.baseLeftFriction.ToString("0.######", Settings.InternalNumberFormat)
					Me.theOutputFileStreamWriter.WriteLine(line)
					...
					line += aBone.theJiggleBone.baseMinUp.ToString("0.######", Settings.InternalNumberFormat)
					line += " "
					line += aBone.theJiggleBone.baseMaxUp.ToString("0.######", Settings.InternalNumberFormat)
					Me.theOutputFileStreamWriter.WriteLine(line)
					line = vbTab
					line += vbTab
					line += "up_friction "
					line += aBone.theJiggleBone.baseUpFriction.ToString("0.######", Settings.InternalNumberFormat)
					Me.theOutputFileStreamWriter.WriteLine(line)
					...
					line += aBone.theJiggleBone.baseMinForward.ToString("0.######", Settings.InternalNumberFormat)
					line += " "
					line += aBone.theJiggleBone.baseMaxForward.ToString("0.######", Settings.InternalNumberFormat)
					Me.theOutputFileStreamWriter.WriteLine(line)
					line = vbTab
					line += vbTab
					line += "forward_friction "
					line += aBone.theJiggleBone.baseForwardFriction.ToString("0.######", Settings.InternalNumberFormat)
					Me.theOutputFileStreamWriter.WriteLine(line)
				...
				If (aBone.theJiggleBone.flags And SourceMdlJiggleBone.JIGGLE_IS_BOING) > 0 Then
					...
					line += vbTab
					line += "impact_speed "
					line += aBone.theJiggleBone.boingImpactSpeed.ToString("0.######", Settings.InternalNumberFormat)
					Me.theOutputFileStreamWriter.WriteLine(line)
					line = vbTab
					line += vbTab
					line += "impact_angle "
					' Reverse this: jiggleInfo->data.boingImpactAngle = cos( DEG2RAD( verify_atof( token ) ) );
					line += MathModule.RadiansToDegrees(Math.Acos(aBone.theJiggleBone.boingImpactAngle)).ToString("0.######", Settings.InternalNumberFormat)
					Me.theOutputFileStreamWriter.WriteLine(line)
					line = vbTab
					line += vbTab
					line += "damping_rate "
					line += aBone.theJiggleBone.boingDampingRate.ToString("0.######", Settings.InternalNumberFormat)
					Me.theOutputFileStreamWriter.WriteLine(line)
					line = vbTab
					line += vbTab
					line += "frequency "
					line += aBone.theJiggleBone.boingFrequency.ToString("0.######", Settings.InternalNumberFormat)
					Me.theOutputFileStreamWriter.WriteLine(line)
					line = vbTab
					line += vbTab
					line += "amplitude "
					line += aBone.theJiggleBone.boingAmplitude.ToString("0.######", Settings.InternalNumberFormat)
					Me.theOutputFileStreamWriter.WriteLine(line)
	...
	Private Sub WriteJiggleBoneConstraints(ByVal aBone As SourceMdlBone)
		...
		If (aBone.theJiggleBone.flags And SourceMdlJiggleBone.JIGGLE_HAS_PITCH_CONSTRAINT) > 0 Then
			...
			line += MathModule.RadiansToDegrees(aBone.theJiggleBone.minPitch).ToString("0.######", Settings.InternalNumberFormat)
			line += " "
			line += MathModule.RadiansToDegrees(aBone.theJiggleBone.maxPitch).ToString("0.######", Settings.InternalNumberFormat)
			Me.theOutputFileStreamWriter.WriteLine(line)
			line = vbTab
			line += vbTab
			line += "pitch_friction "
			line += aBone.theJiggleBone.pitchFriction.ToString("0.######", Settings.InternalNumberFormat)
			Me.theOutputFileStreamWriter.WriteLine(line)
			line = vbTab
			line += vbTab
			line += "pitch_bounce "
			line += aBone.theJiggleBone.pitchBounce.ToString("0.######", Settings.InternalNumberFormat)
			Me.theOutputFileStreamWriter.WriteLine(line)
		End If
		...
		If (aBone.theJiggleBone.flags And SourceMdlJiggleBone.JIGGLE_HAS_YAW_CONSTRAINT) > 0 Then
			...
			line += vbTab
			line += "yaw_constraint "
			line += MathModule.RadiansToDegrees(aBone.theJiggleBone.minYaw).ToString("0.######", Settings.InternalNumberFormat)
			line += " "
			line += MathModule.RadiansToDegrees(aBone.theJiggleBone.maxYaw).ToString("0.######", Settings.InternalNumberFormat)
			Me.theOutputFileStreamWriter.WriteLine(line)
			line = vbTab
			line += vbTab
			line += "yaw_friction "
			line += aBone.theJiggleBone.yawFriction.ToString("0.######", Settings.InternalNumberFormat)
			Me.theOutputFileStreamWriter.WriteLine(line)
			line = vbTab
			line += vbTab
			line += "yaw_bounce "
			line += aBone.theJiggleBone.yawBounce.ToString("0.######", Settings.InternalNumberFormat)
			Me.theOutputFileStreamWriter.WriteLine(line)
		End If
		...
		If (aBone.theJiggleBone.flags And SourceMdlJiggleBone.JIGGLE_HAS_ANGLE_CONSTRAINT) > 0 Then
			...
			line += MathModule.RadiansToDegrees(aBone.theJiggleBone.angleLimit).ToString("0.######", Settings.InternalNumberFormat)
			Me.theOutputFileStreamWriter.WriteLine(line)
		End If
	End Sub
	...
	Private Sub WriteHBoxCommands(ByVal theHitboxes As List(Of SourceMdlHitbox), ByVal commentTag As String, ByVal hitboxSetName As String, ByRef theSkipBoneInBBoxCommandWasUsed As Boolean)
		...
		For j As Integer = 0 To theHitboxes.Count - 1
			...
			If Me.theMdlFileData.version >= 49 AndAlso aHitbox.unknown <> 0 Then
				...
				line += " "
				line += aHitbox.boundingBoxPitchYawRoll.z.ToString("0.######", Settings.InternalNumberFormat)
				line += " "
				line += aHitbox.boundingBoxPitchYawRoll.x.ToString("0.######", Settings.InternalNumberFormat)
				line += " "
				line += aHitbox.boundingBoxPitchYawRoll.y.ToString("0.######", Settings.InternalNumberFormat)
				line += " "
				line += aHitbox.unknown.ToString("0.######", Settings.InternalNumberFormat)
			End If
			...
			For Each aBodyGroupPreset As SourceMdlBodygroupPreset In Me.theMdlFileData.theBodygroupPresets
				...
				For bodyPartIndex As Integer = Me.theMdlFileData.theBodyParts.Count - 1 To 0 Step -1
					...
					If aBodyPart.modelCount > 1 AndAlso accumMask >= aBodyPart.base Then
						...
						line += aBodyPart.theName
						line += """ "
						line += currentBodyModelValue.ToString(Settings.InternalNumberFormat)
